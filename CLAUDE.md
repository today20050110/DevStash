...
- If the Neon MCP fails to connect, do not fall back to connection strings from `.env.production`. Use `.env` (Development) or ask me.
- `.env.production` is loaded by `npm run build` and `npm run start`. `build` does not query the database (every data page calls `connection()` and renders dynamically), so it stays part of the normal workflow. `start` serves pages against production — do not run it locally unless I explicitly say "production" in the current request. If a future change makes a page static or adds a build-time query, `build` would hit production too; flag it.
